...
if test "$(id -u)" -gt "0"; then
  ...
  if test ! -d /home/linuxbrew/.linuxbrew; then
    name="$(hostname -s)"
    linuxbrew_home="${XDG_DATA_HOME:-$HOME/.local/share}"/azure-cli/"${name}"
    printf "Setting up Linuxbrew...\t\t\t\t "
    if test ! -d "${linuxbrew_home}"; then
      mkdir -p "${linuxbrew_home}"
      if test -d "${XDG_DATA_HOME:-$HOME/.local/share}"/azure-cli/.linuxbrew; then
        mv "${XDG_DATA_HOME:-$HOME/.local/share}"/azure-cli/.linuxbrew "${linuxbrew_home}"/.linuxbrew
      fi
    fi
  ...
  if test ! -f /etc/linuxbrew.firstrun; then
    for f in /usr/share/azure/homebrew/*.Brewfile; do
      # TODO: use brewfiles from HOME?
      brew bundle --file "$f"
    done

    sudo touch /etc/linuxbrew.firstrun
    printf "\nazure-CLI first run complete!\n\n"
  fi
fi
